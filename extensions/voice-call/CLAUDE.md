...
- **Sandbox tool allowlist blocks `message` by default** — The default sandbox tool allowlist (`src/agents/sandbox/constants.ts` `DEFAULT_TOOL_ALLOW`) does NOT include `message`. Since sandbox `mode: "all"` is enabled globally, the voice agent won't have the `message` tool unless you explicitly add it to the agent's sandbox allowlist in config: `agents.list[].tools.sandbox.tools.allow`. Without this, the LLM will try to use `exec` to shell out `openclaw message send`, which fails (no binary in container). To enable SMS from voice calls, the voice agent config needs: (1) `tools.sandbox.tools.allow` including `"message"`, (2) `tools.deny: ["exec"]` to prevent the exec fallback, (3) global `tools.message.crossContext.allowAcrossProviders: true` because the voice agent's Pi session runs in `webchat` context (hardcoded in `openai-http.ts`) but sends via `twilio-sms`.
- **Twilio SMS webhook path (local vs prod)** — The `twilio-sms` channel plugin registers a single webhook path (default `/twilio-sms`). Locally, Twilio sends to the voice-call webhook server (:3334) which proxies non-voice paths to the gateway. The Twilio webhook URL uses `/text/webhook`, so set `channels.twilio-sms.webhookPath` to `/text/webhook` in config. In production where Twilio hits the gateway directly, use the default `/twilio-sms` or whatever path is configured in the Twilio console. The `twilio-sms` plugin also needs `plugins.entries.twilio-sms.enabled: true` and `channels.twilio-sms.enabled: true` with DM policy (`dm.policy: "open"` or `"pairing"`).
