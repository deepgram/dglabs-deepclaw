  ...
  private async handleGatewayProxy(
    ...
    try {
      ...
      if (contentType?.includes("text/event-stream") && proxyRes.body) {
        ...
        // Static fallback timer — tracked separately so we can cancel it when dynamic arrives
          ...
          if (userMessage) {
            void generateFillerSet({ userMessage }, anthropicKey).then((fillers) => {
              if (fillers && fillers.length > 0) {
                ...
                // Cancel the static fallback — dynamic takes priority
                if (staticTimer) {
                ...
                if (fillerCount === 0) {
                  // Respect the filler threshold — don't inject before the deadline
                  const elapsed = Date.now() - proxyStartTime;
                  const jitter = Math.floor(Math.random() * 1000) - 500; // ±500ms
                  const remaining = Math.max(0, threshold + jitter - elapsed);
                  if (remaining <= 0) {
